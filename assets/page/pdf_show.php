...
{
$name = $_POST['mobile'];
//echo "SELECT * FROM `donate` WHERE `mobile`='$name'and `id`= '$occupantRegno'";die;
 
if(preg_match("/^\d+\.?\d*$/",$name) && strlen($name)==10){
...
$result = mysqli_query($con,"SELECT * FROM `donate` WHERE `mobile`='$name'and `id`= '$occupantRegno'")or die('Error');
...
$pdf->Output("abcd",'I');
...
else
	{
		 ...
		 echo "<script>alert('Valid PHONE Number'); window.location='../../index.php'</script>";die;
